...
if (length(args) < 3) {
  log_error("Insufficient arguments. Usage: Rscript estimate_activity.R <record_table_csv> <species_name> <output_dir> [group_column]")
...
if (!file.exists(record_csv)) {
  log_error("Input not found: %s\nProbable cause: process_camtrap_data.R was not executed or failed\nCheck: whether record_table.csv exists in the output directory\nPrevious skill: camera-trap-processing (process_camtrap_data.R)", record_csv)
  stop("Missing record_csv: ", record_csv)
}

log_decision("species_name", species_name,
             "target species for daily activity estimation")
log_decision("group_col", ifelse(is.null(group_col), "NULL", group_col),
             "group column for overlap comparison; NULL = no comparison")

dir.create(output_dir, recursive = TRUE, showWarnings = FALSE)

log_step(1, "Loading and filtering records for target species")
records <- tryCatch({
  read.csv(record_csv, stringsAsFactors = FALSE)
}, error = function(e) {
  log_error("Failed to read record_table CSV: %s\nProbable cause: corrupted file or invalid format\nCheck: output of process_camtrap_data.R\nPrevious skill: camera-trap-processing", conditionMessage(e))
  stop(e)
})
...
log_info("Records for '%s': %d events", species_name, nrow(sp_data))

if (nrow(sp_data) < 10) {
  log_error("n_independent_events < 10 for '%s' (%d found)\nProbable cause: rare species or independence threshold too high\nCheck: available species in the record table\nPrevious skill: camera-trap-processing (process_camtrap_data.R)", species_name, nrow(sp_data))
  stop("n_independent_events < 10 for '", species_name,
       "'. Report RAI only; do not estimate activity overlap.")
}

log_step(2, "Converting times to radians and estimating activity density")
# Convert to radians
to_rad <- function(dt) {
...
tryCatch({
  ...
  dev.off()
  log_info("Activity plot saved: activity_plot.png")
}, error = function(e) {
  log_error("Failed to generate activity plot: %s\nProbable cause: overlap library issue\nCheck: overlap package installation\nPrevious skill: [none]", conditionMessage(e))
  stop(e)
})

log_step(3, "Computing circular statistics")
# Circular statistics
...
kappa_est  <- tryCatch(mle.vonmises(time_circ)$kappa, error = function(e) {
  log_warn("mle.vonmises() failed (kappa = NA): %s", conditionMessage(e))
  NA_real_
})
...
if (rayleigh_p >= 0.05) {
  log_warn("Rayleigh p = %.4f >= 0.05: daily activity does not differ from uniform for '%s'",
...
log_step(4, "Computing activity overlap between groups (if applicable)")
...
if (!is.null(group_col) && group_col %in% names(sp_data)) {
  ...
  if (length(groups) == 2) {
    ...
    if (length(groupA) >= 10 && length(groupB) >= 10) {
      log_info("Estimating Dhat4 overlap between '%s' (n=%d) e '%s' (n=%d)",
      ...
      dev.off()
      log_info("Dhat4 overlap = %.3f [%.3f, %.3f]",
               delta4, ci_lower, ci_upper)
    } else {
      log_warn("One or both groups have < 10 events; overlap not calculated (n_A=%d, n_B=%d)",
               length(groupA), length(groupB))
    }
  } else {
    log_warn("group_col '%s' has %d unique values; exactly 2 required for overlap",
             group_col, length(groups))
  }
} else if (!is.null(group_col)) {
  log_warn("Group column '%s' not found in species data", group_col)
}
...
log_info("  Mean activity hour: %.1f h", mean_hour %% 24)
